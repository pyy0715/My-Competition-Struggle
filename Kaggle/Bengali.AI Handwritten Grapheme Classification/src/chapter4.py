...
from model import se_resnext50, ResNet34
from criterion import mixup, mixup_criterion, cutmix, cutmix_criterion
...
def evaluate(model, test_loader):
    ...
    with torch.no_grad():
        ...
        final_preds = torch.cat(final_preds)
        final_targets = torch.cat(final_targets)
        recall, grapheme = macro_recall(final_preds, final_targets)

        return final_loss/len(test_loader), recall, grapheme
...
def main():
    ...
    vid_dataset = BengaliDataset(csv=df_train.iloc[vid_idx], img_height=HEIGHT, img_width=WIDTH, transform=transforms_test)
    ...
    for epoch in range(1, EPOCHS+1):
        ...
        val_loss, val_recall, val_grapheme = evaluate(model, vid_loader)

        early_stopping(val_grapheme, model)
        ...
        if val_grapheme > best_recall:
                if not os.path.isdir("checkpoint"):
                    os.makedirs("checkpoint")

                print('###### Model Save, Validation Recall is : {:.4f}, Validation_Grapheme is {:.4f}, Loss is : {:.4f}'.format(
                    val_recall, val_grapheme, val_loss))
                torch.save(model.state_dict(), './checkpoint/final_resnext_saved_weights.pth')
                best_recall = val_grapheme
